...
function findAdkKitCategoryIds(data: unknown): Set<string> {
  const ids = new Set<string>();
  const list = normalizeList(data ?? null);
  for (const c of list) {
    const name = String(c.name ?? c.title ?? "").trim().toLowerCase().replace(/\s+/g, " ");
    if (name.includes("adk kit") || name === "adk") {
      const id = c.id;
...
function productHasAdkKitCategory(product: ProductRow, adkKitIds: Set<string>): boolean {
  if (adkKitIds.size === 0) return false;
  ...
  return cats.some((c: unknown) => {
    ...
    if (typeof c === "object") {
      ...
      return adkKitIds.has(String(obj.id ?? "")) || String(obj.name ?? "").toLowerCase().includes("adk kit");
    }
    return adkKitIds.has(String(c));
  });
}

export function AdkKitProductsPage() {
  ...
  const adkKitIds = useMemo(() => {
    const ids = findAdkKitCategoryIds(catQ.data);
    const allCats = catQ.data ? normalizeList(catQ.data) : [];
    console.log("ADK Kit Products Debug:", {
      catData: catQ.data,
      allCats: allCats.map((c) => ({ id: c.id, name: c.name ?? c.title })),
      adkKitIds: [...ids],
      catLoading: catQ.isLoading,
      catError: catQ.isError,
    });
    return ids;
  }, [catQ.data]);

  const rows = useMemo(() => normalizeList(prodQ.data ?? null) as ProductRow[], [prodQ.data]);

  const adkKitRows = useMemo(() => {
    // Direct match by category ID first
    let matched = rows.filter((r) => productHasAdkKitCategory(r, adkKitIds));
    // Fallback: if no matches by ID, check if any product has a category value matching "adk kit" directly
    if (matched.length === 0) {
      matched = rows.filter((r) => {
        ...
        return cats.some((c: unknown) => {
          if (c == null) return false;
          const val = typeof c === "object" ? String((c as Record<string, unknown>).name ?? "") : String(c);
          return val.toLowerCase().includes("adk kit");
        });
      });
    }
    console.log("ADK Kit Products: filtered rows:", {
      totalProducts: rows.length,
      matchedCount: matched.length,
      allProductCategories: rows.map((r) => ({ id: r.id, title: r.title, categories: r.categories })),
    });
    return matched;
  }, [rows, adkKitIds]);

  const parsedRows = useMemo(
    () => adkKitRows.map((r) => ({ raw: r, ...parseProductRow(r) }) as EnrichedProductRow),
    [adkKitRows],
  );
  ...
  if (prodQ.isLoading && !prodQ.data) return <LoadingScreen message="Loading ADK Kit products…" />;

  if (prodQ.isError) {
    return (
      <div className="rounded-2xl border border-rose-200 bg-white p-6 dark:border-rose-400/30 dark:bg-slate-950">
        <p className="mb-4 font-semibold text-rose-600">Unable to load ADK Kit products.</p>
        <button
          type="button"
          onClick={() => void prodQ.refetch()}
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">ADK Kit Products</h1>
          <p className="mt-1.5 text-sm text-slate-600 dark:text-slate-400">
            Manage ADK Kit products — stock, BV, and prices.
          </p>
        </div>
      ...
      <div className="space-y-3 md:hidden">
        ...
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-slate-100 bg-white p-8 text-center text-slate-500 dark:border-white/5 dark:bg-slate-950">
            No ADK Kit products match your search.
          </div>
        ) : null}
      </div>
      ...
      <div className="hidden md:block">
        <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:border-white/5 dark:bg-slate-950 dark:shadow-none">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 px-5 py-3.5 text-sm text-slate-500 dark:border-white/5 dark:text-slate-400">
            <span>
              Showing {showing} of {total} ADK Kit product{total === 1 ? "" : "s"}
            </span>
            <button
              type="button"
              ...
              onClick={() => {
                setSearch("");
                toast.info("Showing all ADK Kit products.");
              }}
            >
              View all
            </button>
